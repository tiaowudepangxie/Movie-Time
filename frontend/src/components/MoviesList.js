...
const MoviesList = ({
    ...
    // only be created <em>once<em> and will be dependencies for 
    // useEffect
// Functions defined within a component are re-created every time the component is called
// useCallback ensures that the functions themselves will be recreated only when their own dependencies change.
    ...
    // the first argument(the function) is gonna get excutted when the second parameter got changed
    // [] a kinda dependencies
    useEffect(() => {
        retrieveRatings();
    }, [retrieveRatings]);
